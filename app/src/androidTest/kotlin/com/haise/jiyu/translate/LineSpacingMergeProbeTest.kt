...
class LineSpacingMergeProbeTest {
    ...
    fun probe_atWhichLineSpacingDoLinesStopMerging() = runBlocking {
        ...
        for (ratio in listOf(1.0f, 1.1f, 1.2f, 1.3f, 1.4f, 1.5f, 1.6f)) {
            val blocks = OcrEngine(context, BubbleMaskSegmenter(context), BubbleBoxDetector(context), MangaOcrPipeline(context, BubbleBoxDetector(context))).recognize(twoLineBubble(ratio), "English")
            if (blocks.size < 2) {
    ...
    fun probe_theReportedShape_shortFirstLine() = runBlocking {
        ...
        for (ratio in listOf(1.2f, 1.3f, 1.4f, 1.5f)) {
            ...
            val blocks = OcrEngine(context, BubbleMaskSegmenter(context), BubbleBoxDetector(context), MangaOcrPipeline(context, BubbleBoxDetector(context))).recognize(bmp, "English")
            if (blocks.size < 2) {
