...
class MangaBoomersSourceTest {

    private lateinit var server: MockWebServer
    private lateinit var source: MangaBoomersSource
    ...
    private val detailHtml = """
        ...
        <div class="genres-content"><a>Comedy</a></div>
        </body></html>
    """.trimIndent()
    ...
    fun setUp() {
        ...
        server.start()
        source = MangaBoomersSource(redirectingClient(server))
    }
    ...
    @Test
    fun `full detail-chapters-pages flow parses correctly`() = runTest {
    ...
    fun `malformed HTML returns empty list, not an exception`() = runTest {
        ...
        server.start()
        val emptySource = MangaBoomersSource(redirectingClient(server))
