...
class MangaLekSourceTest {

    private lateinit var server: MockWebServer
    private lateinit var source: MangaLekSource
    ...
    fun setUp() {
        ...
        server.start()
        source = MangaLekSource(redirectingClient(server))
    }
    ...
    @Test
    fun `getPopular parses title and cover, language is arabic`() = runTest {
        val result = source.getPopular(1)
        assertEquals(1, result.size)
        assertEquals("Test Series", result[0].title)
        assertEquals("ar", source.language)
    }
    ...
    fun `malformed HTML returns empty list, not an exception`() = runTest {
        ...
        server.start()
        val emptySource = MangaLekSource(redirectingClient(server))
